 ...
 """
    ...
    function getMelodyIndex(mss::MelodySampleSpace, melody::Vector{Int8})::UInt64
        ...
        awspCardinality = length(mss.ASPV)     # Cardinality of ASPV (as a set of unique elements)
        pow = awspCardinality^(Int(mss.n) - 1)      # Max needed pow of ASPV's (set) cardinality
	...
	"""
    function getMelodyOffset(mss::MelodySampleSpace, melodyIndex::UInt64)::Int
        avsp = getMelodyFromIndex(mss, melodyIndex)
        off = 0
        for k in 1:mss.n    
            off += avsp[k]
        end
	...
	When the above SUM equals one of the values in mss.ASPV, we say it is  C I R C U L A R 
	"""
    function getMelodyAttributes(mss::MelodySampleSpace, melodyIndex::UInt64)::Tuple{Bool, Bool}

        off = getMelodyOffset(mss, melodyIndex)
        closed = (off == 0)
        circular = (off in mss.ASPV)
    ...
    """
    This method of the function getMelodyAttributes works directly with the melody's AVSP
    """
    function getMelodyAttributes(mss::MelodySampleSpace, melody::Vector{Int8})::Tuple{Bool, Bool}
        off = sum(melody)
        closed = (off == 0)
        circular = (off in mss.ASPV)
